...
class ManyVids(Bot):
    ...
    def __init__(self, username):
        ...
        self.getVideo = lambda url, file: getVideoNativeHLS(self, url, file)
    ...
    def updateSiteCookies(self):
        r = self.session.get(
            "https://www.manyvids.com/tak-live-redirect.php", allow_redirects=False
        )
        self.cookies.update(r.cookies)
    ...
    def getVideoUrl(self):
        r = self.requestStreamInfo()
        params = json.loads(
            base64.b64decode(r.cookies.get("CloudFront-Policy").replace("_", "="))
        )
    ...
    def getStatus(self):
        try:
            r = self.session.get(
                "https://roompool.live.manyvids.com/roompool/"
                + self.username
                + "?private=false",
                headers=self.headers,
            )
