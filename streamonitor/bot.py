...
class Bot(Thread):
    ...
    def __init__(self, username):
        ...
        use_cs = USE_CLOUDSCRAPER and self.use_cloudscraper
        self.session = create_http_session(use_cs)
        ...
        if "getVideo" not in type(self).__dict__:
            self.getVideo = lambda url, file: getVideoFfmpeg(self, url, file)
    ...
    def handle_status_error(self, e, context="getStatus"):
        if isinstance(e, requests.exceptions.ConnectionError):
            self.logger.debug(f"[{context}] Connection error: {e}")
        elif isinstance(e, requests.exceptions.Timeout):
            self.logger.debug(f"[{context}] Timeout: {e}")
        elif isinstance(e, requests.exceptions.HTTPError):
            self.logger.debug(
                f"[{context}] HTTP {e.response.status_code} {e.response.reason}: {e}"
            )
        elif isinstance(e, (ValueError, KeyError)):
            self.logger.debug(f"[{context}] Parse error: {e}")
        else:
            self.logger.debug(f"[{context}] {type(e).__name__}: {e}")

    def log_response_debug(self, r, context="getStatus"):
        self.logger.debug(
            f"[{context}] HTTP {r.status_code} {r.reason} for {r.url}"
            + (f": {r.text[:200]}" if r.status_code >= 400 else "")
        )
                ...
                try:
                    ...
                    if self.sc == Status.OFFLINE:
                        ...
                        if self.sc == Status.PUBLIC:
                            ...
                            try:
                                ret = self.getVideo(video_url, file)
                            except Exception as e:
                                self.logger.exception(e)
                                ret = False
                            if not ret:
    ...
    def getWantedResolutionPlaylist(self, url):
        try:
            ...
            audio_url = None
            if selected_source.get("audio"):
                self.logger.debug(
                    f"Stream requests audio group: {selected_source['audio']}"
                )
                # We need to re-fetch the master playlist to get the media groups because we stripped them earlier
                master_res = self.session.get(
                    url, headers=self.headers, cookies=self.cookies
                )
                master_doc = master_res.content.decode("utf-8")
                master_m3u8 = m3u8.loads(master_doc)

                for media in master_m3u8.media:
                    self.logger.debug(
                        f"Found media type: {media.type}, group_id: {media.group_id}"
                    )
                    if (
                        media.type == "AUDIO"
                        and media.group_id == selected_source["audio"]
                    ):
                        audio_url = media.uri
                        self.logger.debug(f"Matched audio URI: {audio_url}")
                        break

            playlist_url = urljoin(url, selected_source_url)
            if audio_url:
                return playlist_url, urljoin(url, audio_url)
            return playlist_url
